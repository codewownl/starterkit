...
it('can sort column', function (string $column) {
    ...
    livewire(ListUsers::class)
        ->loadTable()
        ->sortTable($column)
        ->assertCanSeeTableRecords($records->sortBy(fn (User $user) => $user->{$column}), inOrder: true)
        ->sortTable($column, 'desc')
        ->assertCanSeeTableRecords($records->sortByDesc(fn (User $user) => $user->{$column}), inOrder: true);
})->with(['name']);
...
it('can search column', function (string $column) {
    ...
    livewire(ListUsers::class)
        ->loadTable()
        ->searchTable($value)
        ->assertCanSeeTableRecords($records->filter(fn (User $user) => $user->{$column} === $value))
        ->assertCanNotSeeTableRecords($records->filter(fn (User $user) => $user->{$column} !== $value));
})->with(['name']);
